...
if __name__ == '__main__':
    ...
    for itemset, support in result:
        ...
        for i in itemset:
            ...
            dict_key += i
            dict_key += ","
        freq_feat[dict_key[0:-1]] = support
sorted_aspect = sorted(freq_feat, key = lambda k: (-freq_feat[k], k))
i=0
for top_k in sorted_aspect:
    if i < 10:
        print top_k +" : "+ str(freq_feat[top_k])
        i += 1


topk = 0
fo = open('aspect_dictionary.txt', 'w')
for i in sorted_aspect:
    topk += 1
    fo.write(i)
    fo.write(":")
    fo.write(str(freq_feat[i]))
    fo.write("\n")
fo.close()
